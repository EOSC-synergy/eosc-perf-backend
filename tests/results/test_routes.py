...
class TestCreate:
    ...
    @mark.parametrize(
        "body", indirect=True, argvalues=[{"json_field_1": "Content", "time": 10}]
    )
    ...
    @mark.parametrize(
        "body",
        indirect=True,
        argvalues=[{"json_field_1": "Content", "time": 10}, {}],  # Empty body
    )
    ...
    @mark.parametrize(
        "query",
        indirect=True,
        argvalues=[post_query],  # Resource can have multiple results
    )
    @mark.parametrize(
        "body", indirect=True, argvalues=[{"json_field_1": "Content", "time": 10}]
    )
...
@mark.parametrize(
    "result_id", indirect=True, argvalues=[results[0]["id"], results[1]["id"]]
)
class TestGet:
...
class TestClaim:
    ...
    @mark.parametrize(
        "body", indirect=True, argvalues=[{"message": "This is a claim example"}]
    )
    ...
    @mark.parametrize("token_iss", [users[0]["iss"]], indirect=True)
    @mark.parametrize(
        "body", indirect=True, argvalues=[{"message": "This is an example report"}]
    )
    def test_403(self, response_POST):
        """POST method fails 403 if user not registered."""
        assert response_POST.status_code == 403
    ...
    @mark.parametrize(
        "body",
        indirect=True,
        argvalues=[{"bad_field": "This is a bad field to raise 422"}],
    )
...
@mark.parametrize("result_id", indirect=True, argvalues=[results[3]["id"]])
class TestListClaims:
    ...
    def test_200_as_user(self, response_GET, url):
        ...
        for item in response_GET.json["items"]:
            ...
            assert models.Result._claim_report_class.query.filter_by(**item)
    ...
    def test_200_as_admin(self, response_GET, url):
        ...
        for item in response_GET.json["items"]:
            ...
            item.pop("uploader")
            item.pop("resource_type")
            assert models.Result._claim_report_class.query.filter_by(**item).first()

    @mark.parametrize(
        "query", indirect=True, argvalues=[{"upload_before": "3000-01-01"}]
    )
    ...
    @mark.usefixtures("grant_admin")
    @mark.parametrize("query", indirect=True, argvalues=[{"unknown-argument": ""}])
    def test_422(self, response_GET):
        """GET method fails 422 if bad request body."""
        assert response_GET.status_code == 422

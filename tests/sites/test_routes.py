...
class TestCreate:
    ...
    @mark.parametrize(
        "body",
        indirect=True,
        argvalues=[{"name": "s3", "address": "addr2", "description": "Text"}],
    )
    ...
    @mark.parametrize(
        "body",
        indirect=True,
        argvalues=[{"name": sites[0]["name"], "address": sites[0]["address"]}],
    )
...
class TestUpdate:
    ...
    @mark.parametrize(
        "body", indirect=True, argvalues=[{"name": "new_name", "address": "new_addr"}]
    )
...
class TestCreateFlavor:
    ...
    @mark.parametrize(
        "body",
        indirect=True,
        argvalues=[{"name": "flavorN", "description": "FlavorN for siteX"}],
    )
    def test_403(self, response_POST):
        """POST method fails 403 if user not registered."""
        assert response_POST.status_code == 403
    ...
    @mark.parametrize(
        "body",
        indirect=True,
        argvalues=[{"description": "New flavor for site"}, {}],  # Empty body
    )
    def test_422(self, response_POST):
        """POST method fails 422 if missing required."""
        assert response_POST.status_code == 422
